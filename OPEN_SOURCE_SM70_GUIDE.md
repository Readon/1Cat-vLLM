...
SM75 目标 GPU 包括 **T4** 和 **RTX 2080** 系列。SM75 镜像和 SM70 镜像使用
同一个 wheel，因为该 wheel 已经包含多架构的 kernel。主要区别是 SM75
入口脚本默认使用 `FLASH_ATTN`，充分利用 Turing GPU 原生支持的 FlashAttention。
